...
export interface EnrollmentRequestReceivedTemplateData {
    studentName: string;
    schoolName: string;
    courseName: string;
    className?: string;
...
export function getEnrollmentRequestReceivedTemplate(data: EnrollmentRequestReceivedTemplateData): EmailTemplate {
...
<head>
    ...
    <title>Novo pedido de matrícula</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    <tr>
                        <td align="center" style="padding: 40px 40px 20px; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); border-radius: 8px 8px 0 0;">
                            <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: 600;">Pedido de matrícula</h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 32px 40px;">
                            <p style="margin: 0 0 16px; color: #374151; font-size: 16px; line-height: 1.6;">Olá, {{studentName}},</p>
                            <p style="margin: 0 0 16px; color: #374151; font-size: 16px; line-height: 1.6;"><strong>{{schoolName}}</strong> enviou um pedido de matrícula para você.</p>
                            ...
                            <p style="margin: 0 0 24px; color: #374151; font-size: 16px; line-height: 1.6;">Acesse o app para aceitar ou recusar o pedido.</p>
    ...
    const text = `Olá, ${data.studentName},\n\n${data.schoolName} enviou um pedido de matrícula.\n\nCurso: ${data.courseName}\nTurma: ${className}\n\nAcesse o app para aceitar ou recusar:\n${loginUrl}\n`;

    return {
        subject: `${data.schoolName} — pedido de matrícula`,
        html,
        text
    };
}
