...
export type SendBoletoNotificationsResult = {
    invoices: { found: number; sent: number; errors: number };
    charges: { found: number; sent: number; errors: number };
    total: { sent: number; errors: number };
};

export async function runSendBoletoNotifications(): Promise<SendBoletoNotificationsResult> {
    ...
    if (!emailProvider) {
        throw new Error(
            'Nenhum EmailProvider configurado. Configure MAILCHIMP_*, SENDGRID_* ou EMAIL_*'
        );
    }
    ...
    for (const invoiceOrm of recentInvoices) {
        try {
            ...
            if (!invoice || !invoice.boletoUrl) continue;
    ...
    for (const chargeOrm of recentCharges) {
        try {
            ...
            if (!charge || !chargeOrm.asaasInvoiceUrl) continue;
            ...
            const chargeType = charge.chargeType === 'TUITION' ? 'tuition' : 'enrollment';
    ...
    const result: SendBoletoNotificationsResult = {
        invoices: { found: recentInvoices.length, sent: invoicesSent, errors: invoicesErrors },
        charges: { found: recentCharges.length, sent: chargesSent, errors: chargesErrors },
        total: { sent: invoicesSent + chargesSent, errors: invoicesErrors + chargesErrors }
    };

    log.info('[Cron] Envio de notificações concluído', result);
    return result;
}
...
async function main() {
    try {
        ...
        await runSendBoletoNotifications();
        process.exit(0);
    } catch (error) {
